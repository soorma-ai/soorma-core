...
| `/tenant-admin-credentials/rotate` | POST | Rotate the persisted tenant admin API credential |
...
- Tenant-admin routes (`/tenant-admin-credentials/rotate`, `/principals`, `/delegated-issuers`, `/tokens/issue`, `/mappings/evaluate`) require a tenant-bound `X-Identity-Admin-Key` plus `X-Tenant-ID`.
...
- `/tenant-admin-credentials/rotate`, `/principals`, `/delegated-issuers`, `/tokens/issue`, `/mappings/evaluate`: tenant admin key plus `X-Tenant-ID`
...
- Tenant admin API keys are now persisted as rotatable credential records. The service stores only a hashed credential secret at rest and returns the cleartext key only at onboarding or rotation time.
...
## Tenant Admin Credential Lifecycle

- Onboarding creates the first persisted tenant admin credential and returns the cleartext API key once.
- `POST /v1/identity/tenant-admin-credentials/rotate` revokes the currently active credential and returns a newly minted key.
