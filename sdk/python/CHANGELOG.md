...
- Added guidance for using `examples.shared.auth` as a lightweight trusted auth proxy for examples, tests, and developer-owned backends.
- Documented that developer-side onboarding helpers must share the same `IDENTITY_SUPERUSER_API_KEY` value as the target identity-service deployment.

## [0.9.0] - 2026-04-17

### Changed
- Version alignment: bumped to 0.9.0 (SDK release synchronized with monorepo 0.9.0 constituents)
