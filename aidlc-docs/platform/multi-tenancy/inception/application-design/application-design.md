...
This document consolidates all application design artifacts for the multi-tenancy model initiative. The initiative replaces soorma-core's single-tier UUID tenancy with a two-tier model (platform tenant + service tenant + service user) across 7 components in 4 packages.
...
|---|---|---|---|
| C1 | `libs/soorma-common` | Modified | Add `DEFAULT_PLATFORM_TENANT_ID` constant + env override; update `EventEnvelope` docstrings |
| C2 | `libs/soorma-service-common` | **NEW** | `TenancyMiddleware`, `get_tenanted_db`, dependency functions, `PlatformTenantDataDeletion` ABC |
...
| C5 | `services/tracker` | Modified | Column renames + `platform_tenant_id`; adopt middleware; `TrackerDataDeletion` |
